<?php
   ...
   if (isset($_SESSION['user_id'])) {
        ...
        $queryPQR = 'SELECT * FROM pqrs WHERE id = :id';
        $recordsPQR = $conn->prepare($queryPQR);
        $recordsPQR->bindParam(':id',$id);
        $recordsPQR->execute();
        $resultsPQR = $recordsPQR->fetch(PDO::FETCH_ASSOC);
    }
    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="listPQR.php">Lista</a></li>
        ...
        <form class="row g-3" action="editPQR.php" method="POST">
            ...
            <div class="col-md-12">
                <div class="input-group">
                    ...
                    <textarea class="form-control" name="asunto" aria-label="With textarea"><?php echo $resultsPQR['topic'];?></textarea>
                </div>
            </div>
            <div class="col-md-12">
                ...
                <select class="form-select" id="validationUser" required name="usuario[]">
                    ...
                    <?php foreach ($resultsUsers as $key => $value) { ?>
                        ...
                        <?php if($value['id'] == $resultsPQR['user_id']): ?>
                            <option value="<?=$value['id'];?>" selected><?=$value['email'];?></option>
                        <?php endif; ?>
